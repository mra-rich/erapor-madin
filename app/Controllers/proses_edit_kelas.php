...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi input
    if ($id_kelas <= 0 || empty($tingkatan) || empty($angka_kelas) || empty($nama_rombel) || empty($id_wali_kelas)) {
        header("Location: data_kelas.php?status=error&message=Data tidak valid atau kurang lengkap (Pastikan Wali Kelas sudah dipilih)!");
        exit;
    }
    ...
    // 2. Cek apakah nama kelas duplikat dengan id_kelas lain di tingkat yang sama
    $cek_query = "SELECT * FROM kelas WHERE id_tingkat = ? AND nama_kelas = ? AND nama_rombel = ? AND id_kelas != ?";
    $stmt_cek = mysqli_prepare($koneksi, $cek_query);
    mysqli_stmt_bind_param($stmt_cek, "issi", $id_tingkat, $nama_kelas, $nama_rombel, $id_kelas);
    mysqli_stmt_execute($stmt_cek);
    $result_cek = mysqli_stmt_get_result($stmt_cek);

    if (mysqli_num_rows($result_cek) > 0) {
        header("Location: data_kelas.php?status=error&message=Nama kelas sudah terdaftar di entri lain!");
        exit;
    }

    // 3. Update database kelas
    $query = "UPDATE kelas SET nama_kelas = ?, id_tingkat = ?, nama_rombel = ?, id_wali_kelas = ? WHERE id_kelas = ?";
    $stmt = mysqli_prepare($koneksi, $query);
    mysqli_stmt_bind_param($stmt, "sisii", $nama_kelas, $id_tingkat, $nama_rombel, $id_wali_kelas, $id_kelas);

    if (mysqli_stmt_execute($stmt)) {
        require_once 'logger.php';
        catat_log($koneksi, $_SESSION['id_pengguna'], 'Edit Kelas', "Memperbarui kelas ID: $id_kelas menjadi Kelas $nama_kelas $nama_rombel $tingkatan");
        header("Location: data_kelas.php?status=success&message=Data kelas berhasil diperbarui!");
    } else {
        header("Location: data_kelas.php?status=error&message=Gagal memperbarui data kelas: " . mysqli_error($koneksi));
    }
