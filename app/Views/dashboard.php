<?php
// Guard: Jika file ini dipanggil oleh DashboardController, jangan eksekusi
// (DashboardController menggunakan dashboard_content.php, bukan file ini)
if (class_exists('App\\Core\\App', false)) {
    return; // Sudah dihandle oleh OOP controller
}
...
$tahun_aktif = $_SESSION['tahun_ajaran'];
$rapot_tahun = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) as total FROM transaksi_raport WHERE tahun_ajaran = '$tahun_aktif'"))['total'];
...
<script>
    // Animasi Toast masuk saat halaman dimuat
