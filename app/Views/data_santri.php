  ...
  <div class="p-4 border-2 border-transparent mt-14">
    ...
    <div class="mb-4 flex flex-wrap gap-3 items-center bg-white px-4 py-3 rounded-lg border border-gray-200 shadow-sm">
        ...
        <div class="relative w-full md:w-64">
            ...
            <input type="text"
                ...
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full py-3 pl-11 pr-8 transition-all"
                placeholder="Cari santri..."
                hx-get="data_santri.php"
    ...
    <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <nav class="flex items-center gap-1">
            <?php
            // Bangun base URL untuk paginasi (pertahankan parameter lain)
            $params = $_GET;
            unset($params['page']);
            $base_url = 'data_santri.php?' . http_build_query($params) . '&page=';
            ?>

            <!-- Tombol Prev -->
            <?php if ($page > 1): ?>
            <a href="<?= $base_url . ($page - 1) ?>" class="px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-blue-600 transition-colors">
            ...
            if ($start_p > 1): ?>
                <a href="<?= $base_url . 1 ?>" class="px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">1</a>
                <?php if ($start_p > 2): ?><span class="px-2 text-gray-400">…</span><?php endif; ?>
            <?php endif; ?>

            <?php for ($i = $start_p; $i <= $end_p; $i++): ?>
                <?php if ($i == $page): ?>
                <span class="px-3 py-2 text-sm font-bold text-white bg-blue-600 border border-blue-600 rounded-lg"><?= $i ?></span>
                <?php else: ?>
                <a href="<?= $base_url . $i ?>" class="px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-blue-600 transition-colors"><?= $i ?></a>
                <?php endif; ?>
            <?php endfor; ?>

            <?php if ($end_p < $total_pages): ?>
                <?php if ($end_p < $total_pages - 1): ?><span class="px-2 text-gray-400">…</span><?php endif; ?>
                <a href="<?= $base_url . $total_pages ?>" class="px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50"><?= $total_pages ?></a>
            <?php endif; ?>
