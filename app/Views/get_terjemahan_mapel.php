...
try {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($row_mapel = mysqli_fetch_assoc($result_mapel)) {
            ...
            // Fallback ke Google Translate API publik gratis
            $url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=id&tl=ar&dt=t&q=" . urlencode($nama_mapel);
            ...
            if ($httpcode == 200 && $response) {
                ...
                if (isset($result_array[0][0][0])) {
                    $terjemahan = $result_array[0][0][0];
                    echo json_encode(['terjemahan' => $terjemahan]);
                    exit;
                }
            }
